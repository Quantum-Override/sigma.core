...
Sigma.Core is a modern C library using **interface-based architecture** with global const interface instances. All _public_ functionality is accessed through interfaces like `Memory`, `String`, `Collections`, etc. Types are opaque pointers for encapsulation.
...
To record a release:
1. Add a new version block to `changelog.anvl` (e.g. `v0_4_1 @[date="YYYY-MM-DD"] { }`)
...
4. Run `python3 ../tools/clog.py sigma.core` to regenerate `CHANGELOG.md`
...
- **Session continuity**: At session start, read `q-or/wip.anvl` (merged view). If sigma.core has active work, announce feature, completed steps, and `next_action`. To update: edit `q-or/wip/sigma.core.anvl`, then run `python3 tools/wip.py sync`.
...
1. ✅ UPDATE WIP (start) — Edit `q-or/wip/sigma-core.anvl`, mark in-progress, run `wip.py sync`
...
**FR/BR Placement:** FRs and BRs targeting this project MUST live in `sigma.core/feature-reqs/` or `sigma.core/bug-reports/`, not in the reporter's project.
